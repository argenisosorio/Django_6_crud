...
from .forms import Register_externForm
import openpyxl
...
@login_required
def home(request):
    registers = Register_extern.objects.all().order_by('-created_at')
    ...
    return render(request, 'registers_extern/home.html', context)


@login_required
def create_register(request):
    if request.method == 'POST':
        form = Register_externForm(request.POST)
        if form.is_valid():
            ...
            return redirect('registers_extern:home')
    else:
        form = Register_externForm()
    
    context = {
        'form': form,
    }

    return render(request, 'registers_extern/create.html', context)

@login_required
def detail_register(request, pk):
    register = get_object_or_404(Register_extern, pk=pk)
    context = {'register': register}
    return render(request, 'registers_extern/detail.html', context)
...
def update_register(request, pk):
    ...
    if request.method == 'POST':
        form = Register_externForm(request.POST, instance=register)
        if form.is_valid():
            form.save()
            return redirect('registers_extern:detail', pk=register.pk)
    else:
        form = Register_externForm(instance=register)
    ...
    return render(request, 'registers_extern/update.html', context)
...
def delete_register(request, pk):
    ...
    context = {'register': register}
    return render(request, 'registers_extern/delete.html', context)
